...
function BoardLeft(props: Props) {
   ...
   return (
      <div>
         {props.showSideBar && (
            ...
            >
               <div className=' flex items-center border-b-[1px] p-2 border-slate-300 justify-between w-full'>
                  <div className='relative p-5 bg-gradient-to-r from-sky-500 to-indigo-500 w-fit rounded-md h-fit'>
                     <span className='absolute top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] text-white font-bold '>
                        {props.board?.title?.toUpperCase().charAt(0)}
                     </span>
                  </div>
                  <div>
               ...
               <div className='my-3 overflow-y-auto'>
                  <BoardLeftTab currentTab={tab} tab='Boards'>
                     {" "}
                     <FaTrello />
                  </BoardLeftTab>
                  ...
                  <h1 className='text-white font-semibold text-sm px-2 my-2'>
                     Workspace views
                  </h1>
                  <BoardLeftTab currentTab={tab} tab='Table'>
                     {" "}
                     <ImTable2 />
                  </BoardLeftTab>
                  <BoardLeftTab currentTab={tab} tab='Calendar'>
                     {" "}
                     <BsCalendarMinus />
                  </BoardLeftTab>
                  <h1 className='text-white font-semibold text-sm px-2 my-2'>
                  ...
                  <div className='overflow-y-auto max-h-[40vh]'>
                     {props.workspace?.boards?.map((board) => {
                        return (
                           <BoardLeftPreviewItem key={board.id} board={board} />
                        )
                     })}
                  </div>
               </div>
         ...
         {!props.showSideBar && (
            <div
               className={` sticky bg-slate-800 bg-opacity-50 bottom-0 left-0 min-h-[calc(100vh-55px)]
               ${
                  props.showSideBar
